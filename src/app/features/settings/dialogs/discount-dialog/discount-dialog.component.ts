...
export class DiscountDialog implements OnInit {
    ...
    discountForm = this.fb.group({
        ...
        discountType: [{ value: '', disabled: this.isUpdate() }, [Validators.required]], // 1 -> percent 2 -> amount
        value: [{ value: 0, disabled: this.isUpdate() }, [Validators.required]],
        isOccuring: [false, Validators.required],
    ...
    ngOnInit(): void {
        ...
        this.discountService.getById(this.data).pipe(
            ...
            next: (discount: Discount) => {
                ...
                this.getIsOccuringControl().setValue(discount.isRecurring);
                this.getExpiryDateControl().setValue(discount.expiryDate);
            },
    ...
    getIsOccuringControl(): AbstractControl {
        return this.discountForm.get('isOccuring')!;
    }

    getExpiryDateControl(): AbstractControl {
        return this.discountForm.get('expiryDate')!;
    }
    ...
    onSubmit(): void {
        ...
        const request: DiscountRequest = {
            ...
            type: this.getDiscountTypeControl().value as DiscountType,
            isRecurring: this.getIsOccuringControl().value,
            expiryDate: this.getExpiryDateControl().value instanceof Date
                ? this.getExpiryDateControl().value.toISOString()
                : this.getExpiryDateControl().value
        };
        ...
        const message = `${this.translationService.getTranslationForKey("shared.succesfully")} ${action}`;
